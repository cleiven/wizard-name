...
function genName() {
    ...
    let name = commonSyls[Math.floor(Math.random() * commonSyls.length)] + commonSyls[Math.floor(Math.random() * commonSyls.length)];
    for (s of Object.keys(umlautChars)) {
        name = name.replace(s,(s) => Math.random()>0.9 ? umlautChars[s] : s);
    }
    return name;
}
